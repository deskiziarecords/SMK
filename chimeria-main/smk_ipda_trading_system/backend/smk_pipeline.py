...
class SMKPipeline:
    ...
    def _bias(self, df):
        ...
        if m:
            try:
                t = m.detect_bias(df)
                return {'bias': str(t.bias), 'eq': float(t.equilibrium), 'zone': str(t.zone),
                        'coherence': float(t.coherence), 'valid': bool(t.is_valid)}
            except Exception:
                pass
        cur = float(df['close'].iloc[-1])
    ...
    def _ipda(self, df):
        ...
        if m:
            try:
                ...
                return {'phase': str(t.phase), 'eq': float(t.equilibrium),
                        'confidence': float(t.confidence), 'valid': bool(t.is_valid)}
            except Exception:
                pass
    ...
    def _eq_cross(self, df):
        ...
        if m:
            try:
                t = m.process_tick(df)
                return {'zone': str(t.zone), 'cross': bool(t.cross_event),
                        'direction': str(t.cross_direction), 'confidence': float(t.confidence)}
            except Exception:
                pass
    ...
    def _session(self, df):
        ...
        if m:
            try:
                ...
                return {'active': bool(t.is_active), 'name': str(t.session_name or 'DEAD_ZONE'),
                        'killzone': bool(t.is_killzone), 'score': float(t.temporal_efficiency_score),
                        'status': str(t.status)}
            except Exception:
                pass
    ...
    def _swings(self, df):
        ...
        if m and len(df) >= 10:
            try:
                sw = m.scan_pivots(df)
                return {'count': len(sw),
                        'nodes': [{'idx': int(s.index), 'price': float(s.price), 'type': str(s.type)} for s in sw[-6:]]}
            except Exception:
                pass
        ...
        if m and len(df) >= 3:
            try:
                ...
                return {'count': len(gaps), 'active': len(gaps) > 0,
                        'recent': [{'type': g.gap_type, 'top': float(g.top_boundary),
                                    'bot': float(g.bottom_boundary), 'eq': float(g.equilibrium)} for g in gaps[-3:]]}
            except Exception:
                pass
        ...
        if m and len(df) >= 3:
            try:
                ...
                return {'count': len(blocks), 'active': len(blocks) > 0,
                        'recent': [{'type': b.ob_type, 'level': float(b.price_level),
                                    'high': float(b.high_boundary), 'low': float(b.low_boundary),
                                    'score': float(b.displacement_score)} for b in blocks[-2:]]}
            except Exception:
                pass
        return {'count': 0, 'active': False, 'recent': []}
        ...
        if m:
            try:
                ...
                return {'zones': len(zones),
                        'hvn': [{'price': float(z.price_level), 'density': float(z.density_score)}
        ...
        if m and len(df) >= 20:
            try:
                ...
                if t:
                    ...
                    return {'ratio': float(t.volatility_ratio), 'entrapped': bool(t.is_entrapped),
                            'energy': float(t.latent_energy_score), 'stasis': int(t.time_in_stasis),
                            'status': str(t.status)}
            except Exception:
                pass
    ...
    def _displacement(self, cur, df):
        ...
        if m:
            try:
                ...
                return {'is_disp': bool(t.is_displacement), 'dir': int(t.direction),
                        'body_ratio': float(t.body_ratio), 'range_mult': float(t.range_mult),
                        'vetoed': bool(t.is_vetoed), 'status': str(t.status)}
            except Exception:
                pass
    ...
    def _expansion(self, df, dr):
        ...
        if m and len(df) >= 20 and 'atr' in df.columns:
            try:
                ...
                return {'sigma': int(t.sigma_t), 'prob': float(t.expansion_prob),
                        'entrapped': bool(t.is_entrapped), 'target': float(t.target_dol or 0), 'status': str(t.status)}
            except Exception:
                pass
    ...
    def _manipulation(self, df):
        ...
        if m and len(df) >= 20:
            try:
                ...
                return {'active': bool(t.is_active), 'score': float(t.confidence_score),
                        'level': str(t.sweep_level), 'wick': float(t.wick_magnitude), 'status': str(t.status)}
            except Exception:
                pass
    ...
    def _kl(self, df):
        ...
        if m:
            try:
                ...
                return {'score': float(t.divergence_score), 'stable': bool(t.regime_stable),
                        'h_curr': float(t.entropy_current), 'h_ref': float(t.entropy_reference), 'status': str(t.status)}
            except Exception:
                pass
        closes = df['close'].values
    ...
    def _topology(self, df):
        ...
        if m and len(df) >= 10:
            try:
                ...
                return {'h1_score': float(t.h1_persistence_score), 'fractured': bool(t.is_fractured),
                        'islands': int(t.active_islands), 'status': str(t.status)}
            except Exception:
                pass
        closes = df['close'].values
    ...
    def _fusion(self, r):
        ...
        if m:
            try:
                ...
                return {'p_fused': float(t.p_fused), 'confidence': float(t.confidence),
                        'veto_active': bool(t.veto_active), 'active_lambdas': [str(x) for x in (t.active_lambdas or [])],
                        'regime': str(t.regime), 'status': str(t.status)}
            except Exception:
                pass
        bias = r['bias']['bias']
    ...
    def _mandra(self, r):
        ...
        if m:
            try:
                ...
                return {'open': bool(t.is_open), 'delta_e': float(t.energy_delta),
                        'size': float(t.clamped_size), 'regime_stable': bool(t.regime_stable), 'status': str(t.status)}
            except Exception:
                pass
        p = r['fusion']['p_fused']
