...
class SMKPipeline:
    ...
    def _load_modules(self):
        ...
        try_load("order_flow",   lambda: _imp("order_flow_visibility_engine", "OrderFlowVisibilityEngine")())
    ...
    async def step(self):
        ...
        # Initialize veto early to avoid KeyErrors in sensor methods
        r['veto'] = {"decision": "Proceed", "reasons": [], "trade_allowed": True}
        ...
        r['order_flow']    = self._order_flow(df, cur)
        r['amd']           = self._amd(r)
        ...
        # Final Veto decision: merge internal logic with sensor-specific vetoes
        v = self._veto(r)
        r['veto']['decision'] = v['decision']
        r['veto']['trade_allowed'] = v['trade_allowed']
        for reason in v['reasons']:
            if reason not in r['veto']['reasons']:
                r['veto']['reasons'].append(reason)
        ...
        try:
            ...
            if bridge:
                ...
                if r['veto']['decision'] == 'Proceed':
                    r['execution'] = await bridge.evaluate(r, self.raw_bars[:idx+1])
                else:
                    r['execution'] = self._null_execution(r['veto']['decision'])
        except Exception as e:
            r['execution'] = self._null_execution(str(e))

        return _sanitize(r)

    def _null_execution(self, reason):
        return {
            "action": "HALT", "reason": reason, "is_armed": False,
            "lot_size": 0.0, "stop_loss_price": 0.0, "take_profit_price": 0.0,
            "kelly_size": 0.0, "pattern": "", "dominant": "X", "direction": 0,
            "venue_allocation": [], "risk_profile": "", "risk_pips": 0.0,
            "rr_ratio": 0.0, "delta_e": 0.0, "rev_score": 0.0
        }

    def _lambda7_macro(self, r, bar, direction):
        m = self.modules.get("lambda7")
        if m:
            try:
                t = m.step(symbol="EURUSD", direction=direction, current_price=bar['close'],
                           dxy_price=bar.get('dxy', 105.0), spx_price=bar.get('spx', 4500.0))
                r['lambda_7'] = {'score': t.score, 'active': t.active, 'status': t.status,
                                 'dxy_correlation': t.dxy_correlation, 'dxy_veto': t.dxy_veto_triggered,
                                 'signal_valid': t.signal_valid, 'risk_regime': t.risk_regime,
                                 'veto_reason': t.veto_reason}
                if t.dxy_veto_triggered:
                    r['veto']['decision'] = 'Halt'
                    r['veto']['reasons'].append(f"λ₇: {t.veto_reason}")
            except Exception as e:
                print(f"[SMK] λ₇ runtime error: {e}")

    def _lambda8_light_cone(self, r, bar):
        m = self.modules.get("lambda8")
        if m:
            try:
                t = m.step(target_price=bar['close'], dxy_price=bar.get('dxy', 105.0),
                           spx_price=bar.get('spx', 4500.0))
                r['lambda_8'] = {'score': t.score, 'active': t.active, 'status': t.status,
                                 'violation_detected': t.violation_detected, 'violation_type': t.violation_type,
                                 'dxy_z_score': t.dxy_z_score, 'target_z_score': t.target_z_score,
                                 'kill_switch': t.kill_switch_triggered, 'kill_switch_reason': t.kill_switch_reason}
                if t.kill_switch_triggered:
                    r['veto']['decision'] = 'Halt'
                    r['veto']['reasons'].append(f"λ₈: {t.kill_switch_reason}")
            except Exception as e:
                print(f"[SMK] λ₈ runtime error: {e}")
    ...
    def _order_flow(self, df, cur):
        m = self.modules.get("order_flow")
        if m:
            try:
                # Synthetic tick generator if real ticks are missing
                ticks = cur.get("ticks")
                if ticks is None:
                    # Generate synthetic ticks for the visibility engine
                    num_ticks = 200
                    delta_time = np.random.exponential(0.5, num_ticks)
                    price_path = np.linspace(cur["open"], cur["close"], num_ticks)
                    noise = np.random.randn(num_ticks) * (cur["high"] - cur["low"]) * 0.1
                    prices = np.clip(price_path + noise, cur["low"], cur["high"])
                    
                    total_vol = cur["volume"]
                    buy_vols = np.random.randint(1, 100, num_ticks)
                    sell_vols = np.random.randint(1, 100, num_ticks)
                    scale = total_vol / (np.sum(buy_vols) + np.sum(sell_vols) + 1e-9)
                    
                    ticks = pd.DataFrame({
                        'delta_time': delta_time,
                        'price': prices,
                        'buy_vol': buy_vols * scale,
                        'sell_vol': sell_vols * scale,
                        'volume': (buy_vols + sell_vols) * scale
                    })
                
                candle_ohlc = {'high': cur['high'], 'low': cur['low']}
                t = m.process_candle_visibility(ticks, candle_ohlc)
                return {
                    "delta": t.delta,
                    "imbalances": t.imbalances,
                    "is_absorption": t.is_absorption,
                    "burst_density": t.burst_density,
                    "institutional_pulse": t.institutional_pulse,
                    "status": t.status
                }
            except Exception:
                pass
    ...
    def _fusion(self, r):
        ...
        if m:
            try:
                ...
                signals = {
                    "λ1_vol_decay":    {"score": 0.9 if r["vol_decay"]["entrapped"] else 0.2,
                                         "confidence": min(1.0, r["vol_decay"]["energy"] / 50), "veto": False},
                    "λ2_session":      {"score": r["session"]["score"],
                                         "confidence": 0.8 if r["session"]["killzone"] else 0.4, "veto": False},
                    "λ3_harmonic":     {"score": -1.0 if r["harmonic"]["inverted"] else 0.4,
                                         "confidence": 0.75, "veto": r["harmonic"]["inverted"]},
                    "λ4_manipulation": {"score": 0.8 if r["manipulation"]["active"] else -0.2,
                                         "confidence": r["manipulation"]["score"] / 100,
                                         "veto": r["manipulation"]["active"]},
                    "λ5_displacement": {"score": float(r["displacement"]["dir"]),
                                         "confidence": 0.85 if r["displacement"]["is_disp"] else 0.4,
                                         "veto": r["displacement"]["vetoed"]},
                    "λ6_bias":         {"score": bias_score,
                                         "confidence": r["bias"]["coherence"], "veto": False},
                    "λ7_macro":        {"score": r.get('lambda_7', {}).get('score', 0.0),
                                         "confidence": 0.7, "veto": r.get('lambda_7', {}).get('dxy_veto', False)},
                    "λ8_light_cone":   {"score": r.get('lambda_8', {}).get('score', 0.0),
                                         "confidence": 0.8, "veto": r.get('lambda_8', {}).get('kill_switch', False)},
                }
    ...
    def _veto(self, r):
        ...
        # Explicitly check λ7 and λ8 results if they were run
        if r.get('lambda_7', {}).get('dxy_veto'):
            reasons.append(f"λ₇: {r['lambda_7'].get('veto_reason')}")
        if r.get('lambda_8', {}).get('kill_switch'):
            reasons.append(f"λ₈: {r['lambda_8'].get('kill_switch_reason')}")
    ...
    def _sensors(self, r):
        ...
        base_sensors = [
            ...
            {"id": "s15", "name": "ORDER FLOW",     "score": r["order_flow"]["burst_density"], "active": r["order_flow"]["institutional_pulse"]},
        ]

        if 'lambda_7' in r:
            base_sensors.append({'id': 'λ₇', 'name': 'Macro Gate', 'score': r['lambda_7']['score'],
                                 'active': r['lambda_7']['active'], 'layer': 'L5-MACRO', 'status': r['lambda_7']['status']})
        if 'lambda_8' in r:
            base_sensors.append({'id': 'λ₈', 'name': 'Light-Cone', 'score': r['lambda_8']['score'],
                                 'active': r['lambda_8']['active'], 'layer': 'L0-ALPHA', 'status': r['lambda_8']['status']})
        return base_sensors
    ...
    def _blank(self, bar, idx):
        ...
        return {
            ...
            "order_flow":   {"delta": 0.0, "imbalances": [], "is_absorption": False, "burst_density": 0.0, "institutional_pulse": False, "status": "INIT"},
...
def _get_plugins():
    ...
    if _plugin_mgr is None:
        try:
            ...
            _plugin_mgr = plugin_manager.get_plugin_manager()
...
def _get_bridge():
    ...
    if _bridge is None:
        try:
            import aegis_bridge
            _bridge = aegis_bridge.get_bridge()
        except Exception: pass
    return _bridge
