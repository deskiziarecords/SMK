...
const AssetKineticMatrix: React.FC<AssetKineticMatrixProps> = ({ symbol, sentimentData }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d0d0f] p-6 font-mono">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4 mb-6">
        <div>
          <h2 className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em]">Kinetic Alignment</h2>
          <div className="text-2xl font-bold text-zinc-100 tracking-tight">{symbol || 'BTCUSDT'} / AGGREGATE</div>
        </div>
        <div className="text-right">
          <div className="text-[10px] text-zinc-600 font-bold uppercase tracking-wider">System 3 Matrix</div>
          <div className="text-[10px] text-zinc-400">MULTIPLE_TIME_GRANULARITY</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {sentimentData.map((item, index) => {
          ...
          return (
            <div 
              key={index} 
              className="bg-zinc-900/40 border border-zinc-800/50 p-4 rounded relative overflow-hidden group transition-all hover:bg-zinc-900/60"
            >
              ...
              <div className="flex justify-between items-start mb-4">
                <span className="text-sm font-bold text-zinc-300">{item.tf}</span>
                <span 
                ...
                <div className="flex-1 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                    <div 
                        ...
                        style={{ 
                            ...
                            background: `linear-gradient(90deg, #ef4444 0%, #52525b 50%, #10b981 100%)`
                        }}
                    />
                </div>
              ...
              <div className="flex gap-0.5 mt-2">
                {[...Array(10)].map((_, i) => (
                    <div 
                        key={i}
                        className={`h-0.5 flex-1 rounded-sm ${i < (intensity * 10) ? '' : 'bg-white/5'}`}
      ...
      <div className="mt-auto pt-8 border-t border-zinc-800/50 flex justify-between items-end text-zinc-600">
        ...
        <div className="bg-zinc-900/80 px-4 py-2 border border-zinc-800 rounded">
            <div className="text-[8px] uppercase font-bold tracking-widest text-zinc-500 mb-1">Coherence Status</div>
